import { ValoresAgrupados } from "../Modules/DatosAgrupados.js";
...
calcular.addEventListener("click", () => {
  const valoresLi = document.querySelectorAll(".valoresLi");
  const valoresLs = document.querySelectorAll(".valoresLs");
  const frecuencias = document.querySelectorAll(".frecuencias");

  const estadistica = new ValoresAgrupados(valoresLi, valoresLs, frecuencias);

  ui.clearAnswer();
  ui.showAnswer2(estadistica.CalcularMd());
});
...
limpiar.addEventListener("click", () => {
  ...
  limpiarTxt(valoresLi);
  limpiarTxt(valoresLs);
